...
using "PYBIRDMgrUI.py". 
    ...
    def _add_aes_key(self):
        ...
        while invalid_key:
            ...
            _test_key_bytes: bytes = _test_key.encode(TCS_variables.AES_ENCODING)
        ...
        _key_hash = hashlib.sha512(_test_key.encode(TCS_variables.AES_ENCODING)).hexdigest()

        _encoded_key = self._encode_key(_test_key)

        _encoded_key_hash = hashlib.sha512(_encoded_key.encode(TCS_variables.AES_ENCODING)).hexdigest()
    ...
    def _read_key(self) -> str:
        if os.path.exists(TCS_variables.AES_KEY_FILE):
            with open(TCS_variables.AES_KEY_FILE, "r") as f:
                ...
                if hashlib.sha512(
                        _encoded_key_file.encode(TCS_variables.AES_ENCODING)).hexdigest() == _encoded_key_hash_file:
                ...
                if hashlib.sha512(_decoded_key.encode(TCS_variables.AES_ENCODING)).hexdigest() == _key_hash_file:
    ...
    def _encode_key(self, key: str):
        ...
        for passes in range(TCS_variables.AES_KEY_SCRAMBLE_LENGTH):
            _b64_bytes = base64.b64encode(_bytes)
            _bytes = _b64_bytes
            msg = "ENCODING STATUS: " + str(passes + 1) + " / " + str(TCS_variables.AES_KEY_SCRAMBLE_LENGTH)
            print(msg, end="\r")
    ...
    def _decode_key(self, key: str):
        ...
        for passes in range(TCS_variables.AES_KEY_SCRAMBLE_LENGTH):
            ...
            _b64_bytes = _bytes
            # msg = "DECODING STATUS: "+str(passes+1)+" / "+str(TCS_variables.AES_KEY_SCRAMBLE_LENGTH)
            # print(msg,end = "\r")
        str_key: str = _bytes.decode(TCS_variables.AES_ENCODING)
        return (str_key)
